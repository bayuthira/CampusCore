...
pub async fn update_enrollment_status_handler(
    State(pool): State<DbPool>,
    Extension(claims): Extension<TokenClaims>,       // Info user (dosen) yang login
    Path(enrollment_id): Path<Uuid>,                 // ID item KRS yang akan diubah
    ...
    let dosen_pa = sqlx::query!("SELECT id FROM dosen WHERE user_id = $1", logged_in_user_id)
        .fetch_optional(&pool).await?
        .ok_or_else(|| AppError::Forbidden("Hanya dosen yang dapat melakukan aksi ini.".to_string()))?;
    ...
    // Tolak jika Dosen PA mahasiswa ini tidak sama dengan dosen yang sedang login
    // KECUALI jika yang login adalah SUPER_ADMIN
    if mahasiswa.dosen_pa_id != Some(dosen_pa.id) && !claims.roles.contains(&"SUPER_ADMIN".to_string()) {
    ...
    let updated_enrollment = krs_repo::update_enrollment_status_repo(&pool, enrollment_id, payload).await?;

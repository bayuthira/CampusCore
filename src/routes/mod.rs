...
pub fn create_router(pool: DbPool) -> Router {
    ...
    let protected_routes = Router::new()
        ...
        .route(
            ...
            post(handlers::mahasiswa_handler::import_mahasiswa_from_csv_handler)
                .layer(middleware::from_fn(require_role(vec!["SUPER_ADMIN".to_string(), "STAF_AKADEMIK".to_string()]))),
        )
        .route(
            "/api/mahasiswa",
            get(handlers::mahasiswa_handler::get_all_mahasiswa_handler)
                .layer(middleware::from_fn(require_role(vec!["SUPER_ADMIN".to_string(), "STAF_AKADEMIK".to_string(), "DOSEN".to_string()])))
            .post(handlers::mahasiswa_handler::create_mahasiswa_handler
                    .layer(middleware::from_fn(require_role(vec!["SUPER_ADMIN".to_string(), "STAF_AKADEMIK".to_string()])))),
        )
        .route(
            "/api/mahasiswa/{id}",
        ...
        .route(
            "/api/matakuliah",
            get(handlers::matakuliah_handler::get_all_matakuliah_handler)
                .post(handlers::matakuliah_handler::create_matakuliah_handler

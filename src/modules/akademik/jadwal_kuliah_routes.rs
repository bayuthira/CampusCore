...
pub fn jadwal_kuliah_router() -> Router<DbPool> {
    ...
    let read_routes = Router::new()
        .route(
            "/akademik/jadwal-kuliah/template-csv", // <-- TAMBAHAN ROUTE
            get(handler::download_jadwal_csv_template_handler),
        )
        .route(
    ...
    let write_routes = Router::new()
        .route(
            "/akademik/jadwal-kuliah/import-csv", // <-- TAMBAHAN ROUTE
            post(handler::import_jadwal_csv_handler),
        )
        .route(
            "/akademik/jadwal-kuliah",

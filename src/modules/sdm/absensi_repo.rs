...
pub async fn clock_in_repo(
    ...
    let today = OffsetDateTime::now_utc().date(); // <-- Variabel ini sekarang akan dipakai

    // 1. Cek apakah sudah ada rekap untuk hari ini
    ...
    if let Some(rekap) = existing_rekap {
        ...
        // 2. Jika belum ada rekap, buat rekap baru (Gunakan query() + bind())
        ...
        .bind(pegawai_id)
        .bind(today) // <-- Gunakan 'today'
        .bind(status_str)
        .execute(&mut *tx)
        .await?;
    }

    // 3. Masukkan log ClockIn
    let tipe_absensi_str = TipeAbsensi::ClockIn.as_str();
    let new_log_id = sqlx::query_scalar(
        r#"
        INSERT INTO log_absensi (pegawai_id, waktu_absensi, tipe_absensi, latitude, longitude, alamat_absensi)
        VALUES ($1, now(), $2::"TipeAbsensi", $3, $4, $5)
        RETURNING id
        "#,
    )
    .bind(pegawai_id)
...
pub async fn get_my_logs_for_day_repo(
    ...
    let list = sqlx::query_as!(
        LogAbsensi,
        r#"
        SELECT id, pegawai_id, waktu_absensi, tipe_absensi as "tipe_absensi: _", latitude, longitude, alamat_absensi
        FROM log_absensi
        WHERE pegawai_id = $1 AND waktu_absensi >= $2 AND waktu_absensi < $3
